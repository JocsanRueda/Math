...
class Scene5(Scene):
    def construct(self):
        ...
        fn1 = ax.plot(lambda x: 4, x_range=[0, 4], color=ORANGE)
        line1 = ax.get_vertical_line(ax.input_to_graph_point(4, fn1), color=YELLOW)
        area1 = ax.get_area(fn1, [0, 4], color=ORANGE, opacity=0.4)
        form1 = MathTex(r"\frac{\alpha}{2}").scale(0.8).next_to(line1, DOWN, buff=0.3)
        form2 = MathTex(r"y").scale(0.8).next_to(area1, RIGHT, buff=0.3)
        form3 = MathTex(r"f_{_\alpha}(\alpha)").scale(0.8).next_to(area1, UP, buff=0.3)
        form4 = form2.copy()
        ...
        eq1 = (
            MathTex(r"{{y}}", "\cdot", r"\frac {\alpha}{2}", "=1")
            .scale(0.8)
            .to_edge(UP + RIGHT)
            .shift([-1, 0, 0])
        )
        ...
        eq3 = (
            MathTex(r"f_{_\alpha}(\alpha)", "=", r"\frac{ 2 }{ {{ \alpha }} }")
            .scale(0.8)
            .to_edge(UP + RIGHT)
            .shift([-1, 0, 0])
        )
